...
@Controller
@RequestMapping(UserRegistrationController.URL)
public class UserRegistrationController {
    ...
    private UserEntity toEntity(UserRegistrationDto dto) {
        return modelMapper.map(dto, UserEntity.class);
    }

    @GetMapping
    public String getSignup(Model model) {
        model.addAttribute(USER_ATTRIBUTE, new UserRegistrationDto());
        return SIGNUP_VIEW;
    }

    @PostMapping
    public String signup(
            @ModelAttribute(name = USER_ATTRIBUTE) @Valid UserRegistrationDto user,
